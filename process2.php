...
<html lang="en">
<head>
<title>ผลการทำแบบทดสอบ28ข้อ</title>
<meta charset="utf-8">
<meta charset="utf-8">
...
<?php
	...
	$result = mysqli_query($conn, "SELECT id,count FROM `graph2`");
...
while($row = mysqli_fetch_array($result)) {
		if($sex=='title'){
			if($sum>=23){
				if($row['id']=='2'){ mysqli_query($conn, "UPDATE `graph2` SET `count`=$row[count]+1 WHERE id=2"); }
				
			}
			else{
				if($row['id']=='1'){ mysqli_query($conn, "UPDATE `graph2` SET `count`=$row[count]+1 WHERE id=1"); }
				
			}	
		}
		if($sex=='male'){
			if($sum>=23){
				if($row['id']=='4'){ mysqli_query($conn, "UPDATE `graph2` SET `count`=$row[count]+1 WHERE id=4"); }
			}
			else{
				if($row['id']=='3'){ mysqli_query($conn, "UPDATE `graph2` SET `count`=$row[count]+1 WHERE id=3"); }				
			}	
		}
		if($sex=='female'){
			if($sum>=23){
				if($row['id']=='6'){ mysqli_query($conn, "UPDATE `graph2` SET `count`=$row[count]+1 WHERE id=6"); }				
			}
			else{
				if($row['id']=='5'){ mysqli_query($conn, "UPDATE `graph2` SET `count`=$row[count]+1 WHERE id=5"); }
			}	
		}
        }
